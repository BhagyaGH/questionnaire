    ...
    public function saveAnswerAction(Request $request, $id, $user) {
        if ($request->getMethod() == 'POST') {
            ...
            if ($id == 6) {
                ...
                return $this->redirect($this->generateUrl('gene_questionnaire_question_fr', array('id' => 10, 'user' => $user)));
            } else {
                ...
                if ($id == 5) {
                    if ($request->get('answer') == 'A' || $request->get('answer') == 'SA') {
                        return $this->redirect($this->generateUrl('gene_questionnaire_multi_questions_fr', array('user' => $user)));
                    } else {
                        return $this->redirect($this->generateUrl('gene_questionnaire_question_fr', array('id' => 10, 'user' => $user)));
                    }
                }
