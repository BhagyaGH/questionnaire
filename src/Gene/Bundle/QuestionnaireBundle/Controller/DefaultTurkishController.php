...
class DefaultTurkishController extends Controller {

    public function homeAction() {
        $text = $this->getDoctrine()->getManager()->getRepository(
                        'GeneQuestionnaireBundle:QuestionTurkish')->findAll();
        return $this->render('GeneQuestionnaireBundle:Default:index_turkish.html.twig', array('content' => $text));
    }

    public function indexAction($user) {
        return $this->redirect($this->generateUrl('gene_questionnaire_question_tr', array('id' => 1, 'user' => $user)));
    }

    public function questionAction($id, $user) {
        $question = $this->getDoctrine()->getManager()->getRepository(
                        'GeneQuestionnaireBundle:QuestionTurkish')->findOneBy(array('id' => $id));
        return $this->render('GeneQuestionnaireBundle:Default:question_turkish.html.twig', array(
    ...
    public function saveAnswerAction(Request $request, $id, $user) {
        if ($request->getMethod() == 'POST') {
            ...
            if ($id == 6) {
                ...
                return $this->redirect($this->generateUrl('gene_questionnaire_question_tr', array('id' => 10, 'user' => $user)));
            } else {
                ...
                if ($id < 5) {
                    return $this->redirect($this->generateUrl('gene_questionnaire_question_tr', array('id' => $id + 1, 'user' => $user)));
                }
                if ($id == 5) {
                    if ($request->get('answer') == 'A' || $request->get('answer') == 'SA') {
                        return $this->redirect($this->generateUrl('gene_questionnaire_multi_questions_tr', array('user' => $user)));
                    } else {
                        return $this->redirect($this->generateUrl('gene_questionnaire_question_tr', array('id' => 10, 'user' => $user)));
                    }
                }
                if ($id == 10) {
                    return $this->redirect($this->generateUrl('gene_questionnaire_question_tr', array('id' => 11, 'user' => $user)));
                }
                return $this->redirect($this->generateUrl('gene_questionnaire_final_tr'));
            }
        }
    }
    
    public function thankyouAction() {
        return $this->render('GeneQuestionnaireBundle:Default:final_turkish.html.twig');
    }
