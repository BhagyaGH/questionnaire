...
class UserChineseController extends Controller {

    public function chineseAction() {
        $questions = $this->getDoctrine()->getManager()->getRepository(
                        'GeneQuestionnaireBundle:UserQuestionsChinese')->findAll();
        return $this->render('GeneQuestionnaireBundle:User:user_chinese.html.twig', array('questions' => $questions));
    }
    ...
    public function saveUserAction(Request $request) {
        if ($request->getMethod() == 'POST') {
            ...
            return $this->redirect($this->generateUrl('gene_questionnaire_user_q_ch', array('user' => $user->getId())));
        }
    }
